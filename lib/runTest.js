...
module.exports = async function runTest (testContext, testFn) {
  function done () {
    ...
    testContext.failed = why instanceof Error ? why : new Error(why)
    done()
  }
  ...
  try {
    ...
    let benchmark
    await Promise.race([
      new Promise(resolve => testContext.sp.sub('done', () => {
        testContext.print.debug('Done')
        resolve()
      })),
      new Promise((resolve, reject) => {
        try {
          ...
          const isAsync = result && typeof result === 'object' && result.then
          if (testContext.bench) {
            let fn
            if (isAsync) {
              fn = d => testFn(testContext, d && d.resolve)
                .then(d.resolve)
                .catch(err => {
                  testContext.print.debug(err)
                  reject(err)
                })
            } else {
              fn = d => {
                try {
                  return testFn(testContext, d && d.resolve)
                } catch (err) {
                  testContext.print.debug(err)
                  reject(err)
                }
            ...
            const options = { defer: isAsync || testFn.length > 1, fn }
            benchmark = new Benchmark(testContext.name, options)
      ...
      // TODO: can this be replaced with runPool.exec(...).timeout()?
    ...
    // NOTE: would be cool to be able to add assertions for benchmarks
    if (benchmark) {
      testContext.print.debug('Benchmark', testContext.name, benchmark.hz)
      testContext.result.hz = Math.round(benchmark.hz)
    } else if (!testContext.result.passed && assertionCalls === 0) {
    ...
    if (snapshotState && (snapshotState.added || snapshotState.updated)) {
      Object.assign(testContext.result, {
