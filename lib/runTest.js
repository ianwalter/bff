...
module.exports = async function runTest (testContext, testFn) {
  ...
  try {
      ...
      new Promise((resolve, reject) => {
        try {
          ...
          if (bench) {
            let fn
            if (isAsync) {
              fn = async d => testContext.fn(testContext, d.resolve)
            } else {
              fn = d => testContext.fn(testContext, d.resolve)
            }
            const benchmark = new Benchmark(testContext.name, fn)
    ...
    if (!bench && !testContext.result.passed && assertionCalls === 0) {
