...
worker({
  ...
  async pptr (file, context) {
    ...
    // Create a file in the memory filesystem that will use the test runner and
    // the test file to actually run the tests.
    mfs.writeFileSync('main.js', `
      // Import the run function from the test runner (browser build).
      import { run } from '@ianwalter/bff'
    ...
    return new Promise((resolve, reject) => {
      ...
      compiler.run(async err => {
        ...
        try {
          ...
          // filesystem.
          const bundle = mfs.readFileSync(path.resolve('dist/main.js'), 'utf8')

          // Evaluate the bundle code on the Puppeteer page and resolve with the
          // test results.
          resolve(await page.evaluate(bundle))
        } catch (err) {
          reject(err)
        }
      })
